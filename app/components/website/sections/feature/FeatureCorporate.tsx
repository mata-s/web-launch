
import { FeatureSection as FeatureSectionData } from '@/lib/website';

type FeatureCorporateProps = {
  section: FeatureSectionData;
};

export default function FeatureCorporate({ section }: FeatureCorporateProps) {
  return (
    <section className="bg-slate-50 px-5 py-14 sm:px-8 lg:px-12 lg:py-20">
      <div className="mx-auto max-w-7xl">
        <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
          <div>
            {section.eyebrow && (
              <p className="text-xs font-semibold uppercase tracking-[0.3em] text-blue-700">
                {section.eyebrow}
              </p>
            )}
            <h3 className="mt-4 max-w-3xl break-keep text-3xl font-bold leading-tight tracking-tight text-slate-950 sm:text-4xl lg:text-5xl">
              {section.title}
            </h3>
          </div>

          <p className="max-w-2xl text-sm leading-7 text-slate-600 sm:text-base sm:leading-8 lg:justify-self-end">
            {section.subtitle}
          </p>
        </div>

        <div className="mt-10 grid gap-4 md:grid-cols-3">
          {section.items.map((item, index) => (
            <article
              key={item.title}
              className="rounded-2rem border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-lg"
            >
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-700 text-sm font-bold text-white">
                {index + 1}
              </div>

              <h4 className="mt-6 break-keep text-xl font-bold text-slate-950">
                {item.title}
              </h4>

              <p className="mt-3 text-sm leading-7 text-slate-600">
                {item.description}
              </p>
            </article>
          ))}
        </div>
      </div>
    </section>
  );
}