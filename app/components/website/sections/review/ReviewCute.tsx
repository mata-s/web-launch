...
type ReviewCuteProps = {
  section: ReviewSectionData;
};

export default function ReviewCute({ section }: ReviewCuteProps) {
  return (
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-3">
          {section.reviews.map((review, index) => (
            <article
              ...
              className="group rounded-[2.5rem] border border-pink-100 bg-white/90 p-7 shadow-md transition duration-300 hover:-translate-y-2 hover:shadow-xl"
            >
              <div className="flex items-center justify-between gap-4">
                ...
                <div className="rounded-full bg-pink-100 px-4 py-2 text-xs font-bold text-pink-600">
                  ★★★★★
                </div>
              </div>

              <p className="mt-7 text-sm leading-7 text-rose-700">
                “{review.comment}”
              </p>
              ...
              <div className="mt-6">
                <p className="text-base font-bold text-rose-950">{review.name}</p>
                <p className="mt-1 text-sm text-rose-500">{review.role}</p>
              </div>
            </article>
          ))}
        </div>
